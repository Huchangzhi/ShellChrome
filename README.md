...
| `c <uid>` | 点击元素 | `c 1_5` |
| `fc <文字> [编号]` | 点击包含文字的元素（自动搜索） | `fc 登录` / `fc 登录 2` |
| `t <uid> <text>` | 输入文本 | `t 1_10 你好` |
| `ft <文字> <文本> [编号]` | 向包含文字的输入框输入 | `ft 用户名 zhangsan` / `ft 用户名 zhangsan 2` |
...
### 示例 3：使用 fc/ft 快速操作（无需获取 UID）

```
🌐 > o baidu.com

🌐 > fc 百度一下
找到 1 个匹配元素，选择第 1 个：百度一下 [uid_15]
✅ 点击完成

🌐 > ft 搜索 我的内容
找到 1 个匹配元素，选择第 1 个：搜索 [uid_10]
✅ 输入完成
```
